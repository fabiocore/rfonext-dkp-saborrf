...
- **Código de acesso único por par (Principal, leilão)**, gerado pra **todo** Principal marcado como participante, independente de elegibilidade em qualquer item (corrigido em 2026-08-09 — antes pulava gerar código pra quem não batia nível de nenhum item).
...
- **Entrada de código** (movido em 2026-08-09): o campo pra digitar o código fica direto no topo da página **Leilões** (`/leiloes`), logo abaixo do título — não é mais uma página separada (`/codigo` foi removido do menu, e a URL antiga redireciona pra `/leiloes` pra não quebrar link salvo). Digita/cola o código (maiúsculas automáticas) e é redirecionado pra `/oferta/:código`; código inválido cai na mesma tela de erro de sempre. A URL direta `/oferta/:código` continua funcionando igual.
